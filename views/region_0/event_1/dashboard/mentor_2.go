...
func init() {
	Region0_Event1_Dashboard_Mentor2 = &Page{
		OnPreRender: SetEventPersonPageData,
		Title: Render(
			func(ctx *Context) (err error) {
				person := ctx.Data.(*PageData).Person
				return EventDashboardTitle("Mentor " + person.Name.String()).Render(ctx)
			},
		),
		...
		Content: Views{
			&ModelIteratorView{
				...
				GetModelView: EventMentorDetailsView,
			},
		},
	}
}

func EventMentorDetailsView(ctx *Context, model interface{}) (view View, err error) {
