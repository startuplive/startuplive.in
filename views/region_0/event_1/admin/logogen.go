...
func init() {
	Region0_Event1_Admin_ChooseLogo = &Page{
		Title: RegionTitle("Choose Logo"),
		Scripts: Renderers{
			admin.PageScripts,
					...
					return &If{
						...
						Content: Views{
							&Form{
								...
								SubmitButtonText:  "Save Your Logo and Color Design and return",
								OnSubmit:          saveLogo,
							},
...
func saveLogo(form *Form, formModel interface{}, ctx *Context) (string, URL, error) {
	...
	primaryColorIndex, err := strconv.ParseInt(ctx.URLArgs[2], 10, 64)
	if err != nil {
		return "", nil, err
	}
	region.PrimaryColorIndex.Set(primaryColorIndex)

	secondaryColorIndex, err := strconv.ParseInt(ctx.URLArgs[3], 10, 64)
	if err != nil {
		return "", nil, err
	}
	region.SecondaryColorIndex.Set(secondaryColorIndex)

	fillPatternIndex, err := strconv.ParseInt(ctx.URLArgs[4], 10, 64)
	if err != nil {
		return "", nil, err
	}
...
func EmbeddLogo() View {
	return RenderView(
		func(ctx *Context) error {
			context, err := Region0_Event1_Admin_ChooseLogo_SVG.GetContext(ctx)
			if err != nil {
				return err
			}
			debug.Nop()
			ctx.Response.XML.OpenTag("div").Attrib("class", "svgcontainer").Attrib("style", "display:", "block")
			RenderTemplate(Region0_Event1_Admin_ChooseLogo_SVG.Filename, ctx.Response, context)
...
func LogoGeneratorColorGrid() View {
	return RenderView(
		func(ctx *Context) error {
			...
			primary, _ := strconv.Atoi(ctx.URLArgs[2])
			secondary, _ := strconv.Atoi(ctx.URLArgs[3])
			...
			for regions.Next(&r) {
				...
				regionsMap[fmt.Sprintf("%d %d", int(region.PrimaryColorIndex), int(region.SecondaryColorIndex))] = &region
			...
			for s := 0; s < 10; s++ {
				...
				for p := 0; p < 10; p++ {
					...
					if reg := regionsMap[fmt.Sprintf("%d %d", primary, secondary)]; reg != nil {
						ctx.Response.XML.OpenTag("td").Attrib("style", "background:", "#cccccc;", "text-align:", "center;", "vertical-align:", "middle;", "color:", "white")
						ctx.Response.XML.Content(reg.Name.Get())
						ctx.Response.XML.CloseTag() //td
					} else {
